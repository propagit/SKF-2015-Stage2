...
class Page extends CI_Controller 
{
	...
	function page_new($id)
	{
		$id = mysql_real_escape_string($id);
        $page = $this->Menu_model->getpage($id);
        $data['page']=$page;
		$data['bands'] = $this->Music_band_model->get_page_bands($id);
	...
	function index_boot()
	{
		$data['all']=$this->News_sticker_model->get();
		$data['music_bands'] = $this->Music_band_model->get_homepage_bands();
		$this->load->view('common/header_boot',$data);
	}
	...
	function _old_index()
	{
		...
		$data['featuring'] = $this->Music_band_model->get_featuring_band();
		$this->load->model('News_sticker_model');
		$data['youtube'] = $this->News_sticker_model->get_youtube();
		$data['adspace'] = $this->News_sticker_model->get_banners();
		$data['music_bands'] = $this->Music_band_model->get_homepage_bands();
		$data['footeradspace'] = $this->News_sticker_model->get_footer_banners();
		$this->load->view('common/header',$data);
...
E: ademel@portphillip.vic.gov.au";
		...
		$this->email->to($email);
		$this->email->bcc('peter_yo11@hotmail.com');
...
Membership Type: ".$cate."<br/>";
	...
	function save_vote()
	{
		$band = $_POST['band_voted'];
		$email = $_POST['email_vote'];
		
		if($this->Music_band_model->vote_redundant_email($email) == 0)
		{
			$data['bandvote_id'] = $band;
			$data['email'] = $email;
			$data['status'] = 1;
			$this->Music_band_model->add_vote($data);
			
			$this->session->set_flashdata('msg', 'Thank you for your vote');
		}
		else
		{
			$this->session->set_flashdata('msg', 'Your email is only can use once');
		}
	...
	function search_bands_key()
	{
		$key = $_POST['key'];
		$this->session->set_userdata('key_band',$key);
		redirect('page/search_bands');
	}
	
	function search_bands($num=9,$row=0)
	{
		$key = $this->session->userdata('key_band');
		$key = mysql_real_escape_string($key);
		//echo $key;
		$data['key'] = $key;
		$data['menus'] = $this->Menu_model->get_menus();
		$data['footeradspace'] = $this->News_sticker_model->get_footer_banners();
		$data['bands'] = $this->Music_band_model->search_band_name($key,$num,$row);
		$count = $this->Music_band_model->search_band_name_count($key);
		
		$this->load->library('pagination');
		$config['base_url'] = base_url()."page/search_bands/".$num."/";
...
General Information: <a href='http://www.stkildafestival.com.au/callforentries/uploads/Doc34.pdf'>Download Here</a><br/><br/>

General Conditions: <a href='http://www.stkildafestival.com.au/callforentries/uploads/Doc37.pdf'>Download Here</a><br/><br/>

Food Trader Checklist : <a href='http://www.stkildafestival.com.au/callforentries/uploads/Doc36.pdf'>Download Here</a><br/><br/>
		";
		...
		$this->email->bcc('peter_yo11@hotmail.com , skftraders@portphillip.com.au , skftraders@portphillip.vic.gov.au'); 
...
Food Act: <a href='".base_url().'uploads/'.$data['food_act']."'>Download Here</a><br/><br/>
...
General Information : <a href='http://www.stkildafestival.com.au/callforentries/uploads/Doc02.pdf'>Download Here</a><br/><br/>

General Conditions : <a href='http://www.stkildafestival.com.au/callforentries/uploads/Doc03.pdf'>Download Here</a><br/><br/>

Extended Trading Checklist : <a href='http://www.stkildafestival.com.au/callforentries/uploads/Doc04.pdf'>Download Here</a><br/><br/>";
...
General Information: <a href='http://www.stkildafestival.com.au/callforentries/uploads/Doc27.pdf'>Download Here</a><br/><br/>

General Information - Market Stall: <a href='http://www.stkildafestival.com.au/callforentries/uploads/Doc24.pdf'>Download Here</a><br/><br/>

Market Trader Checklist: <a href='http://www.stkildafestival.com.au/callforentries/uploads/Doc26.pdf'>Download Here</a><br/><br/>
		";
		...
		$this->email->cc($data['email']);
		$this->email->bcc('peter_yo11@hotmail.com , skftraders@portphillip.com.au , skftraders@portphillip.vic.gov.au'); 
		//$this->email->bcc('peter_yo11@hotmail.com','skftraders@portphillip.vic.gov.au');
	...
	{
		$data['links'] = $this->Menu_model->get_links(1,0);
		$data['gallery'] = $this->Gallery_model->get_gallery(25);
		if(!$data['gallery'])
			{
				redirect(base_url());
			}
		...
		$data['photos'] = $this->Gallery_model->get_photos($data['gallery']['id']);
        $this->load->view('common/header',$data);
	...
	function banner($id)
	{
		...
		{
			$this->load->model('News_sticker_model');
		    $banner = $this->News_sticker_model->get_banner($id);
			if($banner)
			{
				$this->News_sticker_model->update_banner($banner['id'],array('clicked' => $banner['clicked'] + 1));
				if(!filter_var($banner['url'], FILTER_VALIDATE_URL))
				{
					redirect(base_url());
				}
				else
	...
	function band($id)
	{
	  ...
	  {	
		...
		$data['footeradspace'] = $this->News_sticker_model->get_footer_banners();
		/*$this->load->view('common/header',$data);
		$this->load->view('pages/band');
		$this->load->view('common/right');
		$this->load->view('common/footer');	*/
		//$this->load->view('common/header-page');
		$this->load->view('common/header',$data);
		$this->load->view('pages/band-new',$data);
		$this->load->view('common/footer');
		}
		else
		{
